...
def mpi_entrance(path_r, path_w):
    ...
    for n in range(len(channel2_data)):
        ...
        entropyArray = np.empty(shape=(neighbor_num-10, 2), dtype=float)
        k = 0
        neighborList = []
        ...
        for j in range(neighbor_num):
            ...
            if j > 9:
                ...
                try:
                    covMatrix = np.cov(neighborMatrix.T)
                    ...
                    if isinstance(eigen_vals[0], complex) or isinstance(eigen_vals[1], complex):
                        ...
                        entropy = - eigen_vals_0_nor * ln_0 - eigen_vals_1_nor * ln_1 - eigen_vals_2_nor * ln_2
                        entropyArray[k, 0] = j
                        entropyArray[k, 1] = entropy

                except:
                    a = 0
                k = k + 1
        optimalNeighborNum = entropyArray[np.argmin(entropyArray[:, 1]), 0]
        optimalNeighbor = neighbor[0:int(optimalNeighborNum)+1, :]
        # if len(optimalNeighbor) == neighbor_num:
        #     nbrs_small = NearestNeighbors(n_neighbors=10, algorithm='kd_tree').fit(neighbor[:, 1:4])
        #     dis, ind = nbrs_small.kneighbors(np.array([centerPoint[0, 1:4]]))
        #     optimalNeighbor = neighbor[ind[0, :]]
