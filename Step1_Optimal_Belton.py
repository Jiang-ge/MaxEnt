    ...
    for n in range(len(channel2_data)):
        ...
        for j in range(neighbor_num):
            ...
            if j > 2:
                ...
                k = k + 1
        sumCurvature = 0
        meanCurvature = np.mean(curvatureArray[:, 1])
        for i in range(neighbor_num-3):
            if i > 0:
                sumCurvature = sumCurvature + pow(curvatureArray[i, 1] - meanCurvature, 2)
                curvatureArray[i, 2] = sumCurvature / i
                if sumCurvature / i > 0.00001:
                    break
            else:
                curvatureArray[i, 2] = 0
        optimalNeighborNum = curvatureArray[i, 0]
